    ...
    <div class="min-h-screen bg-gradient-to-br from-green-200 to-blue-300">
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div class="flex justify-between items-center mb-8">
                <div>
                    <h1 class="text-3xl font-bold text-indigo-800">Edit Category: <span class="text-indigo-600">{{ $category->name }}</span></h1>
                    <p class="mt-2 text-sm text-indigo-600">Update this product category information</p>
                </div>
                <a href="{{ route('categories.index') }}" class="inline-flex items-center px-4 py-2 border border-indigo-300 text-sm font-medium rounded-full shadow-sm text-indigo-700 bg-white hover:bg-indigo-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-all duration-200">
            ...
            <div class="bg-white shadow-xl rounded-2xl overflow-hidden border border-indigo-100">
                <form action="{{ route('categories.update', $category->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                    <div class="p-8 space-y-8 bg-violet-100">
                        <!-- Name Field -->
                        <div>
                                ...
                                <select name="parent_id" id="parent_id"
                                    ...
                                    @foreach($categoriesTree as $parent)
                                        @include('dashboard.admin.categories.partials.category-option', [
                                            'category' => $parent,
                                            'depth' => 0,
                                            'oldValue' => old('parent_id', $category->parent_id)
                                        ])
                                    @endforeach
                                </select>
                                <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                        ...
                        <div>
                            ...
                            <div class="mt-1 relative">
                                <select name="status" id="status" required
                                        class="block w-full pl-4 pr-10 py-3 text-base border-2 border-indigo-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm bg-white shadow-sm transition-all duration-200">
                                    <option value="active" {{ old('status', $category->status) == 'active' ? 'selected' : '' }} class="text-green-600">Active</option>
                                    <option value="inactive" {{ old('status', $category->status) == 'inactive' ? 'selected' : '' }} class="text-amber-600">Inactive</option>
                                </select>
                                <div class="absolute inset-y-0 right-0 pr-3 flex items-center pointer-events-none">
                                    <svg class="h-5 w-5 text-indigo-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 9l4-4 4 4m0 6l-4 4-4-4"></path>
                    ...
                    <div class="px-8 py-6 bg-indigo-50 border-t border-indigo-100 flex justify-end space-x-4">
                        <a href="{{ route('categories.index') }}" class="inline-flex items-center px-6 py-3 border border-indigo-300 text-base font-medium rounded-full shadow-sm text-indigo-700 bg-white hover:bg-indigo-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 transition-all duration-200">
                            Cancel
                        </a>
