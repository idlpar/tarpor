            ...
            function renderAttributesTable(attributes) {
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-5 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Group</th>
                                    <th class="px-5 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Unit</th>
                ...
                if (attributes.length === 0) {
                    ...
                    attributes.forEach(attribute => {
                        const statusClass = attribute.deleted_at ? 'bg-red-200 text-red-900' : 'bg-green-200 text-green-900';
                        const statusText = attribute.deleted_at ? 'Trashed' : 'Active';
                        const trashedBadge = attribute.deleted_at ? '<span class="px-2 py-1 rounded-full text-xs font-semibold bg-gray-200 text-gray-800 ml-2">Trashed</span>' : '';
                        ...
                        tableHtml += `
                            <tr class="border-b border-gray-200 text-sm hover:bg-amber-50" data-id="${attribute.id}">
                                ...
                                <td class="px-5 py-5 whitespace-nowrap">${attribute.group ? attribute.group.name : 'N/A'}</td>
                                <td class="px-5 py-5 whitespace-nowrap">${attribute.unit ?? 'N/A'}</td>
                                ...
                                <td class="px-5 py-5 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex items-center justify-end space-x-2">
                                        <a href="/product-specifications/attributes/${attribute.id}/edit" class="text-indigo-600 hover:text-indigo-900 custom-tooltip-trigger" data-tooltip="Edit Attribute">
                                        ...
                                        ${attribute.deleted_at ? `
                                            <form action="/product-specifications/attributes/${attribute.id}/restore" method="POST" class="inline restore-form" onsubmit="return false;">
                                                @csrf
                                                @method('PATCH')
                                            ...
                                            <span class="text-gray-300">|</span>
                                            <form action="/product-specifications/attributes/${attribute.id}/force-delete" method="POST" class="inline force-delete-form" onsubmit="return false;">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="text-red-600 hover:text-red-900 custom-tooltip-trigger" data-tooltip="Force Delete Attribute">
                                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                                                </button>
                                            </form>
                                        ` : `
                                            <form action="/product-specifications/attributes/${attribute.id}" method="POST" class="inline delete-form" onsubmit="return false;">
                                                @csrf
                                                @method('DELETE')
            ...
            attributesTableContainer.addEventListener('submit', async function(event) {
                if (event.target.classList.contains('delete-form') ||
                    ...
                    if (form.classList.contains('restore-form')) {
                        ...
                        text = 'This action cannot be undone. The attribute will be permanently deleted.';
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            try {
                                ...
                                console.error(`Error ${method}ing attribute:`, error);
