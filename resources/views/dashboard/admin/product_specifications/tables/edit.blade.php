...
@section('admin_content')
    <div class="container mx-auto p-4">
        ...
        <div class="flex justify-between items-center mb-4">
            <h1 class="text-2xl font-bold">Edit Product Specification Table</h1>
        </div>

        <div class="bg-white shadow-md rounded-lg p-6">
            <form action="{{ route('admin.product_specifications.tables.update', $table->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="mb-4">
                    <label for="name" class="block text-gray-700 text-sm font-bold mb-2">Table Name:</label>
                    <input type="text" name="name" id="name" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" value="{{ old('name', $table->name) }}" required>
                    @error('name')
                    ...
                    <div class="bg-gray-100 p-4 rounded-lg">
                        <ul id="sortable-groups" class="space-y-2">
                            ...
                            @foreach($table->groups as $group)
                                <li id="group-item-{{ $group->id }}" class="bg-white p-2 rounded shadow-sm flex justify-between items-center">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Initial population of sortable list based on selected groups
            @if(old('groups'))
                ...
                updateHiddenInputs();
            @else
                // If no old input, populate from existing table groups
                @foreach($table->groups->sortBy('pivot.order') as $group)
