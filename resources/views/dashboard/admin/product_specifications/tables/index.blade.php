...
@endpush
    <div class="container mx-auto px-4 py-4">
        ...
        <x-ui.content-card>
            <div id="tables-table-container">
                <!-- Tables table content will be loaded here via AJAX -->
            </div>
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            const tablesTableContainer = document.getElementById('tables-table-container');
            ...
            async function fetchTablesData(page = 1) {
                loadingSpinner.style.display = 'block';
                tablesTableContainer.innerHTML = ''; // Clear previous content
                ...
                try {
                    ...
                    renderTablesTable(data.tables.data);
                    renderPagination(data.tables, paginationContainer, fetchTablesData);

                } catch (error) {
                    console.error('Error fetching table data:', error);
                    tablesTableContainer.innerHTML = '<p class="text-red-500 text-center py-4">Failed to load tables. Please try again.</p>';
                } finally {
                    loadingSpinner.style.display = 'none';
                }
            }

            // Function to render tables table
            function renderTablesTable(tables) {
                        ...
                        <table class="min-w-full divide-y divide-gray-200">
                            <thead class="bg-gray-50">
                                <tr>
                                    ...
                                    <th class="px-5 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Groups</th>
                ...
                if (tables.length === 0) {
                    ...
                    tables.forEach(table => {
                        const statusClass = table.deleted_at ? 'bg-red-200 text-red-900' : 'bg-green-200 text-green-900';
                        const statusText = table.deleted_at ? 'Trashed' : 'Active';
                        const trashedBadge = table.deleted_at ? '<span class="px-2 py-1 rounded-full text-xs font-semibold bg-gray-200 text-gray-800 ml-2">Trashed</span>' : '';
                        const groupsHtml = table.groups.map(group => `<span class="inline-block bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold text-gray-700 mr-2 mb-2">${group.name}</span>`).join('') || 'N/A';

                        tableHtml += `
                            <tr class="border-b border-gray-200 text-sm hover:bg-amber-50" data-id="${table.id}">
                                <td class="px-5 py-5 whitespace-nowrap">${table.name}</td>
                                <td class="px-5 py-5 whitespace-nowrap">${table.type.charAt(0).toUpperCase() + table.type.slice(1)}</td>
                                <td class="px-5 py-5 whitespace-nowrap">${groupsHtml}</td>
                                ...
                                <td class="px-5 py-5 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex items-center justify-end space-x-2">
                                        <a href="/product-specifications/tables/${table.id}/edit" class="text-indigo-600 hover:text-indigo-900 custom-tooltip-trigger" data-tooltip="Edit Table">
                                        ...
                                        ${table.deleted_at ? `
                                            <form action="/product-specifications/tables/${table.id}/restore" method="POST" class="inline restore-form" onsubmit="return false;">
                                                @csrf
                                                @method('PATCH')
                                            ...
                                            <span class="text-gray-300">|</span>
                                            <form action="/product-specifications/tables/${table.id}/force-delete" method="POST" class="inline force-delete-form" onsubmit="return false;">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="text-red-600 hover:text-red-900 custom-tooltip-trigger" data-tooltip="Force Delete Table">
                                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                                                </button>
                                            </form>
                                        ` : `
                                            <form action="/product-specifications/tables/${table.id}" method="POST" class="inline delete-form" onsubmit="return false;">
                                                @csrf
                                                @method('DELETE')
                ...
                `;
                tablesTableContainer.innerHTML = tableHtml;
            }
            ...
            // Highlight table row if redirected from edit/create
            const highlightTableId = {{ session('highlight_table_id') ?? 'null' }};
            if (highlightTableId) {
                const checkTableInterval = setInterval(() => {
                    const tableRow = tablesTableContainer.querySelector(`tr[data-id="${highlightTableId}"]`);
                    if (tableRow) {
            ...
            tablesTableContainer.addEventListener('submit', async function(event) {
                if (event.target.classList.contains('delete-form') ||
                    ...
                    if (form.classList.contains('restore-form')) {
                        title = 'Restore Table?';
                        text = 'This will restore the table.';
                        confirmButtonText = 'Yes, restore it!';
                        icon = 'info';
                    } else if (form.classList.contains('force-delete-form')) {
                        title = 'Permanently Delete Table?';
                        text = 'This action cannot be undone. The table will be permanently deleted.';
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            try {
                                ...
                                console.error(`Error ${method}ing table:`, error);
