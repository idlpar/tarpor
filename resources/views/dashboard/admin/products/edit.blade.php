            ...
            <form action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="w-full flex flex-col lg:flex-row gap-6" id="productForm">
                ...
                <div class="w-full lg:w-9/12">
                    ...
                    <div class="bg-white p-8 rounded-lg shadow-lg">
                        ...
                        <!-- Description -->
                        <x-forms.tinymce id="description" name="description" value="{{ old('description', $product->description) }}">Description</x-forms.tinymce>

                        <!-- Content (short_description) -->
                        <x-forms.tinymce id="short_description" name="short_description" value="{{ old('short_description', $product->short_description) }}">Content</x-forms.tinymce>
                        ...
                        <x-form.card label="Product FAQs" class="bg-transparent">
                            ...
                            <div id="faq-input-container" class="d-none space-y-4">
                                <div class="faq-repeater-group" data-next-index="0">
                                    ...
                                    @if(isset($product) && $product->faqs->isNotEmpty())
                                        @foreach($product->faqs as $index => $faq)
                                            <div class="faq-item bg-gray-50 p-4 rounded-lg border border-gray-200 relative">
                                                <div class="mb-3">
                                                    ...
                                                    <textarea class="w-full border border-gray-300 p-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" name="new_faqs_data[{{ $index }}][question]" rows="1">{{ old('new_faqs_data.' . $index . '.question', $faq->question) }}</textarea>
                                                </div>
                                                <div class="mb-3">
                                                    <label class="block font-semibold text-gray-700 mb-2">Answer</label>
                                                    <textarea class="w-full border border-gray-300 p-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" name="new_faqs_data[{{ $index }}][answer]" rows="1">{{ old('new_faqs_data.' . $index . '.answer', $faq->answer) }}</textarea>
                                                </div>
                                                ...
                                                <li>
                                                    <label class="form-check flex items-center space-x-2 py-1">
                                                        <input type="checkbox" id="selected-existing-faqs-item-{{ $faq->id }}" name="selected_faqs[]" class="form-checkbox h-5 w-5 text-blue-600 rounded" value="{{ $faq->id }}"
                                                            @if(isset($product) && in_array($faq->id, old('selected_faqs', $product->faqs->pluck('id')->toArray()))) checked @endif>
...
@push('scripts')
        ...
        document.addEventListener('DOMContentLoaded', () => {
            let galleryManager = {
                ...
                initPreloadedImages() {
                    ...
                    console.log('productMedia from Laravel:', productMedia); // Add this line
                    if (productMedia && productMedia.length > 0) {
                        ...
                        // If no featured image was explicitly found, and there are gallery images,
                        // we don't automatically set the first gallery image as featured and remove it.
                        // The featured image should be managed separately.
                        // The productImages array should only contain gallery images.
                        // If a featured image needs to be displayed, it should be loaded from $product->thumbnail_media
                        // and set via setFeaturedImage without affecting productImages.
                        // The current logic for setting featured image from $product->thumbnail_media in initPreloadedImages
                        // already handles this correctly if $product->thumbnail_media is set.
                        // The problematic part was removing a gallery image if no featured image was found.
                        // This block is now effectively removed as it was causing unintended side effects on productImages.
            ...
            async function handleSubmit(event) {
                ...
                // The images_existing input is handled by the galleryManager updating the hidden input field.
                // New image files are appended as images_new[].
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            function filterCategories() {
                ...
                const allLis = categoryTree.querySelectorAll('li');

                // Reset all highlights and show all items if search is empty
                allLis.forEach(li => {
                    const label = li.querySelector('.category-label');
                    if (label) {
                        label.innerHTML = label.textContent;
                    }
                    li.style.display = searchTerm === '' ? 'block' : 'none';
                });

                if (searchTerm === '') return;

                // Find and process matching items
                const matchingLabels = Array.from(categoryTree.querySelectorAll('.category-label'))
                    .filter(label => label.textContent.trim().toLowerCase().includes(searchTerm));
                ...
                matchingLabels.forEach(label => {
                    ...
                    // Show the parent li and all its ancestors up to the tree root
                    let current = label.closest('li');
                    while (current && current !== categoryTree) {
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            let faqIndex = faqRepeaterGroup.children.length; // Initialize index based on pre-existing FAQs
