...
@section('admin_content')
    <div class="container mx-auto px-4 py-6">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
            <div>
                <h1 class="text-2xl font-bold text-text-dark">Edit Collection</h1>
        ...
        <div class="bg-white p-8 rounded-lg shadow-lg">
            <form action="{{ route('collections.update', $collection->id) }}" method="POST">
                ...
                <div class="mb-6">
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name', $collection->name) }}" class="w-full border border-input-border bg-input-bg text-text-dark p-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary @error('name') border-error @enderror" placeholder="Collection Name">
                    @error('name')
                ...
                <div class="mb-6">
                    ...
                    <input type="text" id="slug" name="slug" value="{{ old('slug', $collection->slug) }}" class="w-full border border-input-border bg-input-bg text-text-dark p-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary @error('slug') border-error @enderror" placeholder="collection-slug">
                    @error('slug')
                    <p class="text-error text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-6">
                    ...
                    <textarea id="description" name="description" class="w-full border border-input-border bg-input-bg text-text-dark p-3 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary @error('description') border-error @enderror" placeholder="Collection Description">{{ old('description', $collection->description) }}</textarea>
                    @error('description')
                    <p class="text-error text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex justify-end">
                    <button type="submit" class="bg-primary text-white px-6 py-2 rounded-lg hover:bg-primary-dark transition-colors">Update Collection</button>
                </div>
            </form>
        </div>
    </div>
