    ...
    <div class="container mx-auto px-4 py-4">
        ...
        <x-ui.content-card>
            <div id="collections-table-container">
                <!-- Collections table content will be loaded here via AJAX -->
            </div>
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            const collectionsTableContainer = document.getElementById('collections-table-container');
            ...
            async function fetchCollectionsData(page = 1) {
                loadingSpinner.style.display = 'block';
                collectionsTableContainer.innerHTML = ''; // Clear previous content
                ...
                try {
                    ...
                    renderCollectionsTable(data.collections.data);
                    renderPagination(data.collections, paginationContainer, fetchCollectionsData);

                } catch (error) {
                    console.error('Error fetching collection data:', error);
                    collectionsTableContainer.innerHTML = '<p class="text-red-500 text-center py-4">Failed to load collections. Please try again.</p>';
                } finally {
                    loadingSpinner.style.display = 'none';
                }
            }

            // Function to render collections table
            function renderCollectionsTable(collections) {
                ...
                if (collections.length === 0) {
                    ...
                    collections.forEach(collection => {
                        const statusClass = collection.status === 'active' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800';

                        tableHtml += `
                            <tr class="border-b border-gray-200 text-sm hover:bg-amber-50" data-id="${collection.id}">
                                <td class="px-5 py-5 whitespace-nowrap">${collection.name}</td>
                                <td class="px-5 py-5 whitespace-nowrap">${collection.slug}</td>
                                <td class="px-5 py-5 whitespace-nowrap">${collection.description ? collection.description.substring(0, 50) + (collection.description.length > 50 ? '...' : '') : ''}</td>
                                ...
                                <td class="px-5 py-5 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex items-center justify-end space-x-2">
                                        <a href="/collections/${collection.id}/edit" class="text-indigo-600 hover:text-indigo-900 custom-tooltip-trigger" data-tooltip="Edit Collection">
                                        ...
                                        <form action="/collections/${collection.id}" method="POST" class="inline delete-form" onsubmit="return false;">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="text-red-600 hover:text-red-900 custom-tooltip-trigger" data-tooltip="Delete Collection">
            ...
            // Highlight collection row if redirected from edit/create
            const highlightCollectionId = {{ session('highlight_collection_id') ?? 'null' }};
            if (highlightCollectionId) {
                const checkTableInterval = setInterval(() => {
                    const collectionRow = collectionsTableContainer.querySelector(`tr[data-id="${highlightCollectionId}"]`);
                    if (collectionRow) {
                        clearInterval(checkTableInterval);
                        collectionRow.classList.add('highlight-row');
                        setTimeout(() => {
                            collectionRow.classList.remove('highlight-row');
                        }, 5000);
                    }
                }, 100);
            }
            ...
            collectionsTableContainer.addEventListener('submit', async function(event) {
                        ...
                        if (result.isConfirmed) {
                            try {
                                ...
                                if (data.success) {
                                    ...
                                    Swal.fire('Error!', data.message || 'Failed to delete collection.', 'error');
                                }
                            } catch (error) {
                                console.error('Error deleting collection:', error);
                                Swal.fire('Error!', error.message || 'An error occurred while deleting the collection.', 'error');
                            }
                        }
                    });
                }
